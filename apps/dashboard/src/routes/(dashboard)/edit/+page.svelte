<script lang="ts">
  ...
  const tabs = [
    ...
    },
    {
      id: "links",
      label: "Liens",
      icon: "M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.826a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1",
    },
    {
      id: "appearance",
      label: "Apparence",
...
<div class="flex flex-col h-full text-slate-200 overflow-hidden">
  <!-- HEADER -->
  <header
    class="h-16 shrink-0 border-b border-white/5 backdrop-blur-md flex items-center justify-between px-6 z-20"
  >
    <div class="flex items-center gap-4">
      ...
      <div class="flex flex-col">
        <h1 class="text-sm font-semibold text-white truncate max-w-50">
          {plinkk.slug || "Éditeur"}
        </h1>
        <div class="flex items-center gap-1.5">
          <div
            class="size-1.5 rounded-full {isSaving
    ...
    <div class="flex items-center gap-3">
      <button
        class="px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-white text-xs font-semibold transition-all border border-white/10"
      >
        Voir la page
      </button>
      <button
        class="px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-xs font-semibold transition-all shadow-lg shadow-violet-900/20"
      >
        Publier
      </button>
    </div>
  ...
  <main
    class="col-span-12 lg:col-span-10 grid grid-cols-1 xl:grid-cols-12 gap-8"
  >
        ...
        <ul class="flex gap-2 min-w-max">
          {#each tabs as tab}
            <button
              ...
              class="w-full flex items-center gap-3 p-3 rounded-xl transition-all group {activeTab ===
              tab.id
            ...
            >
              <div
                class="size-10 sm:size-6 flex items-center justify-center shrink-0"
              >
                <svg
        ...
        <div class="max-w-2xl mx-auto pb-20">
          ...
          {:else if activeTab === "background"}
            <BackgroundSection
              {plinkk}
              onupdate={(field, value) => updatePlinkk(field, value)}
              onupdateGradient={(data) =>
                updatePlinkk("background", data.colors)}
            />
    ...
    <aside
      class="hidden xl:block xl:col-span-5 relative space-y-6 max-h-screen"
    >
      <div class="sticky top-8">
        ...
        >
          ...
          <div class="flex justify-center gap-2 mb-2 z-10 relative">
            <button
              id="btnPreview"
              class="px-3 py-1 text-sm font-medium rounded-lg bg-violet-600 text-white"
              >Aperçu</button
            >
            <button
              id="btnHistory"
              class="px-3 py-1 text-sm font-medium rounded-lg bg-slate-700 text-slate-200"
              >Historique</button
            >
          </div>
          <div
          ...
          <!-- <div class="mt-8 flex flex-col items-center gap-2">
            <div
              class="flex items-center gap-2 text-slate-500 text-xs font-medium"
            >
              <svg
                class="w-4 h-4"
                viewBox="0 0 24 24"
            ...
            <p class="text-[10px] text-slate-600 text-center max-w-50">
              Certaines modifications peuvent prendre quelques secondes à
              apparaître dans l'aperçu.
            </p>
          </div> -->
        </div>
      </div>
